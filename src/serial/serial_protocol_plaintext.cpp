...
void SerialProtocolPlaintext::handleState(const PB_SmartKnobState& state) {
    ...
    if (substantial_change) {
        // todo stream_.printf("STATE: %d [%d, %d]  (detent strength: %0.2f, width: %0.0f deg, endstop strength: %0.2f)\n", 
...
void SerialProtocolPlaintext::log(const char* msg) {
    // todo stream_.print("LOG: ");
    // stream_.println(msg);
}

void SerialProtocolPlaintext::loop() {
    // todo while (stream_.available() > 0) {
    //     int b = stream_.read();
    //     if (b == 0) {
